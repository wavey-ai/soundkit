...
// Modified by SoundKit in 2026 for bounded, buffered bit reads.
...
use super::huffman_tree::{PeekedDataLookupResult, VorbisHuffmanTree};
...
macro_rules! bpc_read_body {
    ($rettype:ident, $bitnum:expr, $octetnum:expr, $selfarg:expr) => {{
        let _ = $octetnum;
        $selfarg.read_bits($bitnum).map(|value| value as $rettype)
    }};
}
...
macro_rules! bpc_peek_body {
    ($rettype:ident, $bitnum:expr, $octetnum:expr, $selfarg:expr) => {{
        let _ = $octetnum;
        $selfarg.peek_bits($bitnum).map(|value| value as $rettype)
    }};
}
...
macro_rules! ik_reader {
    ($fnname:ident, $rettype:ident, $bitnum_of_rettype:expr, $bitnum:expr, $octetnum:expr) => {
        ...
        pub fn $fnname(&mut self) -> Result<$rettype, ()> {
            Ok(sign_extend!(
                r#try!(bpc_read_body!($rettype, $bitnum, $octetnum, self)),
                $rettype,
                $bitnum_of_rettype,
                $bitnum
            ))
...
macro_rules! ik_dynamic_reader {
    ($fnname:ident, $rettype:ident, $bitnum_of_rettype:expr) => {
        ...
        pub fn $fnname(&mut self, bit_num: u8) -> Result<$rettype, ()> {
            ...
            Ok(sign_extend!(
                r#try!(bpc_read_body!($rettype, bit_num, octet_num, self)),
                $rettype,
                $bitnum_of_rettype,
                bit_num
            ))
...
impl<'a> BitpackCursor<'a> {
    #[inline(always)]
    fn peek_bits(&self, bit_num: u8) -> Result<u64, ()> {
        if bit_num == 0 {
            return Ok(0);
        }
        let bit_num = bit_num as usize;
        let end = self.bit_position.checked_add(bit_num).ok_or(())?;
        if end > self.inner.len().saturating_mul(8) {
            return Err(());
        }

        let byte_position = self.bit_position >> 3;
        let shift = self.bit_position & 7;
        if bit_num + shift <= 64 {
            let word = if byte_position + 8 <= self.inner.len() {
                u64::from_le_bytes(
                    self.inner[byte_position..byte_position + 8]
                        .try_into()
                        .unwrap(),
                )
            } else {
                let mut word = 0u64;
                for (offset, &byte) in self.inner[byte_position..].iter().enumerate() {
                    word |= u64::from(byte) << (offset << 3);
                }
                word
            };
            let mask = if bit_num == 64 {
                u64::MAX
            } else {
                (1u64 << bit_num) - 1
            };
        ...
        let byte_count = (shift + bit_num + 7) >> 3;
        let mut word = 0u128;
        for (offset, &byte) in self.inner[byte_position..byte_position + byte_count]
            .iter()
            .enumerate()
        {
            word |= u128::from(byte) << (offset << 3);
        }
        let mask = (1u128 << bit_num) - 1;
        Ok(((word >> shift) & mask) as u64)
    }

    #[inline(always)]
    fn read_bits(&mut self, bit_num: u8) -> Result<u64, ()> {
        let value = self.peek_bits(bit_num)?;
        self.bit_position += bit_num as usize;
        Ok(value)
    }

    #[inline(always)]
    fn advance_bits(&mut self, bit_num: u8) -> Result<(), ()> {
        let end = self.bit_position.checked_add(bit_num as usize).ok_or(())?;
        if end > self.inner.len().saturating_mul(8) {
            return Err(());
        }
    ...
    pub fn read_bit_flag(&mut self) -> Result<bool, ()> {
        return Ok(r#try!(self.read_u1()) == 1);
    }
    ...
    pub fn read_huffman(&mut self, tree: &VorbisHuffmanTree) -> Result<u32, ()> {
        let mut iter = match self.peek_u8() {
            Ok(data) => match tree.lookup_peeked_data(8, data as u32) {
                PeekedDataLookupResult::Iter(advance, iter) => {
                    r#try!(self.advance_dyn_u8(advance));
                    iter
                }
        ...
        loop {
            let bit = r#try!(self.read_bit_flag());
            if let Some(value) = iter.next(bit) {
                return Ok(value);
            }
        }
    }
