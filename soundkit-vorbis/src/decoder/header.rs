...
// Modified by SoundKit in 2026 for in-tree header decoding.
...
use super::bitpacking::BitpackCursor;
use super::header_cached::{compute_bark_map_cos_omega, CachedBlocksizeDerived};
use super::huffman_tree::{HuffmanError, VorbisHuffmanTree};
use super::ilog;
use std::error;
use std::fmt;
...
impl From<Error> for HeaderReadError {
    fn from(err: Error) -> HeaderReadError {
        match err.kind() {
            ...
            _ => panic!(
                "Non EOF Error occured when reading from Cursor<&[u8]>: {}",
                err
            ),
        }
    }
...
impl fmt::Display for HeaderReadError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        let description = match self {
            ...
            HeaderReadError::BufferNotAddressable => {
                "Requested to create buffer of non-addressable size"
            }
        };
...
macro_rules! convert_to_usize {
    ( $val:expr, $val_type:ident ) => {{
        ...
        if $val != converted as $val_type {
            r#try!(Err(HeaderReadError::BufferNotAddressable));
        }
        converted
    }};
}
...
( $rdr:expr ) => { {
	...
	if res & 1 == 0 {
		...
		r#try!(Err(HeaderReadError::HeaderIsAudio));
	}
	let is_vorbis =
		r#try!($rdr.read_u8()) == 0x76 && // 'v'
		r#try!($rdr.read_u8()) == 0x6f && // 'o'
		r#try!($rdr.read_u8()) == 0x72 && // 'r'
		r#try!($rdr.read_u8()) == 0x62 && // 'b'
		r#try!($rdr.read_u8()) == 0x69 && // 'i'
		r#try!($rdr.read_u8()) == 0x73;   // 's'
	if !is_vorbis {
		r#try!(Err(HeaderReadError::NotVorbisHeader));
	}
	return Ok(res);
}}
}
...
fn test_read_hdr_begin() {
    ...
    let test_arr = &[
        0x01, 0x76, 0x6f, 0x72, 0x62, 0x69, 0x72, /**/ 0x00, 0x00, 0x00, 0x00, 0x02, 0x44,
        0xac, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xb5, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
        0xb8, 0x01,
    ];
    let mut rdr: BitpackCursor = BitpackCursor::new(test_arr);
    assert_eq!(
        read_header_begin(&mut rdr),
...
pub fn read_header_ident(packet: &[u8]) -> Result<IdentHeader, HeaderReadError> {
    ...
    let hd_id = r#try!(read_header_begin(&mut rdr));
    if hd_id != 1 {
        r#try!(Err(HeaderReadError::HeaderBadType(hd_id)));
    }
    let vorbis_version = r#try!(rdr.read_u32());
    if vorbis_version != 0 {
        r#try!(Err(HeaderReadError::UnsupportedVorbisVersion));
    }
    let audio_channels = r#try!(rdr.read_u8());
    let audio_sample_rate = r#try!(rdr.read_u32());
    let bitrate_maximum = r#try!(rdr.read_i32());
    let bitrate_nominal = r#try!(rdr.read_i32());
    let bitrate_minimum = r#try!(rdr.read_i32());
    let blocksize_0 = r#try!(rdr.read_u4());
    let blocksize_1 = r#try!(rdr.read_u4());
    let framing = r#try!(rdr.read_u8());
    if blocksize_0 < 6
        || blocksize_0 > 13
        || blocksize_1 < 6
        || blocksize_1 > 13
        ...
        || audio_sample_rate == 0
    {
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }
...
fn test_read_header_ident() {
    // Valid ident header
    let test_arr = &[
        0x01, 0x76, 0x6f, 0x72, 0x62, 0x69, 0x73, 0x00, 0x00, 0x00, 0x00, 0x02, 0x44, 0xac, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xb5, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb8, 0x01,
    ];
...
pub fn read_header_comment(packet: &[u8]) -> Result<CommentHeader, HeaderReadError> {
    ...
    let hd_id = r#try!(read_header_begin_cursor(&mut rdr));
    if hd_id != 3 {
        r#try!(Err(HeaderReadError::HeaderBadType(hd_id)));
    }
    // First read the vendor string
    let vendor_length = r#try!(read_cursor_u32_le(&mut rdr)) as usize;
    ...
    r#try!(rdr.read_exact(&mut vendor_buf));
    let vendor = r#try!(String::from_utf8(vendor_buf));

    // Now read the comments
    let comment_count = r#try!(read_cursor_u32_le(&mut rdr)) as usize;
    let mut comment_list = Vec::with_capacity(comment_count);
    for _ in 0..comment_count {
        let comment_length = r#try!(read_cursor_u32_le(&mut rdr)) as usize;
        ...
        r#try!(rdr.read_exact(&mut comment_buf));
        ...
        let eq_idx = match comment.find("=") {
            ...
            None => continue, // r#try!(Err(HeaderReadError::HeaderBadFormat))
        };
    ...
    let framing = r#try!(read_cursor_u8(&mut rdr));
    if framing != 1 {
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }
...
fn read_cursor_u32_le(cursor: &mut Cursor<&[u8]>) -> Result<u32, HeaderReadError> {
    let mut bytes = [0u8; 4];
    cursor.read_exact(&mut bytes)?;
    Ok(u32::from_le_bytes(bytes))
}

fn read_cursor_u8(cursor: &mut Cursor<&[u8]>) -> Result<u8, HeaderReadError> {
    let mut byte = [0u8; 1];
...
impl ResidueBook {
    pub fn get_val(&self, i: u8) -> Option<u8> {
        if i >= 8 {
            ...
            panic!(
                "Tried to get ResidueBook value out of bounds (index = {})",
                i
            );
        }
    ...
    fn read_book(
        ...
        for i in 0..7 {
            ...
            let val_entry = r#try!(rdr.read_u8());
...
fn lookup_vec_val_decode(
    lup: &CodebookVqLookup,
    codebook_entries: u32,
    codebook_dimensions: u16,
) -> Vec<f32> {
    let mut value_vectors =
        Vec::with_capacity(codebook_entries as usize * codebook_dimensions as usize);
    if lup.codebook_lookup_type == 1 {
        ...
        for lookup_offset in 0..codebook_entries {
            ...
            for _ in 0..codebook_dimensions {
                let multiplicand_offset =
                    (lookup_offset / index_divisor as u32) as usize % codebook_lookup_values;
                let vec_elem = lup.codebook_multiplicands[multiplicand_offset] as f32
                    * lup.codebook_delta_value
        ...
        for lookup_offset in 0..codebook_entries {
            let mut last = 0.;
            let mut multiplicand_offset: usize =
                lookup_offset as usize * codebook_dimensions as usize;
            for _ in 0..codebook_dimensions {
                let vec_elem = lup.codebook_multiplicands[multiplicand_offset] as f32
                    * lup.codebook_delta_value
                    + lup.codebook_minimum_value
...
impl<'a> BitpackCursor<'a> {
    ...
    pub(crate) fn read_huffman_vq<'b>(
        &mut self,
        b: &'b Codebook,
    ) -> Result<&'b [f32], HuffmanVqReadErr> {
...
static MAX_BASES_WITHOUT_OVERFLOW: &[u32] = &[
    0xffffffff, 0xffffffff, 0x0000ffff, 0x00000659, 0x000000ff, 0x00000054, 0x00000028, 0x00000017,
    0x0000000f, 0x0000000b, 0x00000009, 0x00000007, 0x00000006, 0x00000005, 0x00000004, 0x00000004,
    0x00000003, 0x00000003, 0x00000003, 0x00000003, 0x00000003, 0x00000002, 0x00000002, 0x00000002,
...
static MAX_BASE_MAX_BITS_WITHOUT_OVERFLOW: &[u8] = &[
    0x1f, 0x1f, 0x0f, 0x0a, 0x07, 0x06, 0x05, 0x04, 0x03, 0x03, 0x03, 0x02, 0x02, 0x02, 0x02, 0x02,
...
fn read_codebook(rdr: &mut BitpackCursor) -> Result<Codebook, HeaderReadError> {
    ...
    let sync_pattern = r#try!(rdr.read_u24());
    if sync_pattern != 0x564342 {
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }

    // 2. Read the _dimension, _entries fields and the ordered bitflag
    let codebook_dimensions = r#try!(rdr.read_u16());
    let codebook_entries = r#try!(rdr.read_u24());
    let ordered = r#try!(rdr.read_bit_flag());

    // 3. Read the codeword lengths
    let mut codebook_codeword_lengths =
        Vec::with_capacity(convert_to_usize!(codebook_entries, u32));
    if !ordered {
        let sparse = r#try!(rdr.read_bit_flag());
        for _ in 0..codebook_entries {
            let length = if sparse {
                let flag = r#try!(rdr.read_bit_flag());
                if flag {
                    r#try!(rdr.read_u5()) + 1
                } else {
        ...
        let mut current_length = r#try!(rdr.read_u5()) + 1;
        while current_entry < codebook_entries {
            let number = r#try!(rdr.read_dyn_u32(ilog((codebook_entries - current_entry) as u64)));
    ...
    let codebook_lookup_type = r#try!(rdr.read_u4());
    ...
    let codebook_lookup: Option<CodebookVqLookup> = if codebook_lookup_type == 0 {
        ...
        let codebook_minimum_value = r#try!(rdr.read_f32());
        let codebook_delta_value = r#try!(rdr.read_f32());
        let codebook_value_bits = r#try!(rdr.read_u4()) + 1;
        let codebook_sequence_p = r#try!(rdr.read_bit_flag());
        ...
        let mut codebook_multiplicands =
            Vec::with_capacity(convert_to_usize!(codebook_lookup_values, u64));
        for _ in 0..codebook_lookup_values {
            codebook_multiplicands.push(r#try!(rdr.read_dyn_u32(codebook_value_bits)));
        }
    ...
    return Ok(Codebook {
        ...
        codebook_huffman_tree: r#try!(VorbisHuffmanTree::load_from_array(
...
fn read_floor(
    ...
    let floor_type = r#try!(rdr.read_u16());
    match floor_type {
        0 => {
            let floor0_order = r#try!(rdr.read_u8());
            let floor0_rate = r#try!(rdr.read_u16());
            let floor0_bark_map_size = r#try!(rdr.read_u16());
            let floor0_amplitude_bits = r#try!(rdr.read_u6());
            ...
            let floor0_amplitude_offset = r#try!(rdr.read_u8());
            let floor0_number_of_books = r#try!(rdr.read_u4()) + 1;
            let mut floor0_book_list =
                Vec::with_capacity(convert_to_usize!(floor0_number_of_books, u8));
            for _ in 0..floor0_number_of_books {
                let value = r#try!(rdr.read_u8());
        ...
        1 => {
            let floor1_partitions = r#try!(rdr.read_u5());
            ...
            for _ in 0..floor1_partitions {
                let cur_class = r#try!(rdr.read_u4());
            ...
            for _ in 0..maximum_class + 1 {
                floor1_class_dimensions.push(r#try!(rdr.read_u3()) + 1);
                let cur_subclass = r#try!(rdr.read_u2());
                floor1_class_subclasses.push(cur_subclass);
                if cur_subclass != 0 {
                    let cur_masterbook = r#try!(rdr.read_u8());
                ...
                for _ in 0..cur_books_cnt {
                    ...
                    let cur_book = (r#try!(rdr.read_u8()) as i16) - 1;
            ...
            let floor1_multiplier = r#try!(rdr.read_u2()) + 1;
            let rangebits = r#try!(rdr.read_u4());
            ...
            for cur_class_num in &floor1_partition_class_list {
                for _ in 0..floor1_class_dimensions[*cur_class_num as usize] {
                    floor1_x_list.push(r#try!(rdr.read_dyn_u32(rangebits)));
                }
            }
            // Now do an uniqueness check on floor1_x_list
            // to check decodability.
            let mut floor1_x_list_sorted = floor1_x_list
                .iter()
                .cloned()
                .enumerate()
                .collect::<Vec<_>>();
...
fn read_residue(
    ...
    let residue_type = r#try!(rdr.read_u16());
    if residue_type > 2 {
        // Undecodable by spec
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }
    let residue_begin = r#try!(rdr.read_u24());
    let residue_end = r#try!(rdr.read_u24());
    ...
    let residue_partition_size = r#try!(rdr.read_u24()) + 1;
    let residue_classifications = r#try!(rdr.read_u6()) + 1;
    let residue_classbook = r#try!(rdr.read_u8());
    ...
    for _ in 0..residue_classifications {
        let mut high_bits = 0;
        let low_bits = r#try!(rdr.read_u3());
        let bitflag = r#try!(rdr.read_bit_flag());
        if bitflag {
            high_bits = r#try!(rdr.read_u5());
        }
    ...
    for cascade_entry in &residue_cascade {
        residue_books.push(r#try!(ResidueBook::read_book(
            rdr,
            *cascade_entry,
            codebooks
        )));
...
fn read_mapping(
    ...
    let mapping_type = r#try!(rdr.read_u16());
    if mapping_type > 0 {
        // Undecodable per spec
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }
    let mapping_submaps = match r#try!(rdr.read_bit_flag()) {
        true => r#try!(rdr.read_u4()) + 1,
        false => 1,
    };
    let mapping_coupling_steps = match r#try!(rdr.read_bit_flag()) {
        true => r#try!(rdr.read_u8()) as u16 + 1,
        false => 0,
    };
    ...
    for _ in 0..mapping_coupling_steps {
        let cur_mag = r#try!(rdr.read_dyn_u8(audio_chan_ilog));
        let cur_angle = r#try!(rdr.read_dyn_u8(audio_chan_ilog));
    ...
    let reserved = r#try!(rdr.read_u2());
    if reserved != 0 {
        // Undecodable per spec
        r#try!(Err(HeaderReadError::HeaderBadFormat));
    }
    ...
    for _ in 0..mapping_submaps {
        ...
        r#try!(rdr.read_u8());
        let cur_floor = r#try!(rdr.read_u8());
        let cur_residue = r#try!(rdr.read_u8());
...
fn read_mode_info(rdr: &mut BitpackCursor, mapping_count: u8) -> Result<ModeInfo, HeaderReadError> {
    let mode_blockflag = r#try!(rdr.read_bit_flag());
    let mode_windowtype = r#try!(rdr.read_u16());
    let mode_transformtype = r#try!(rdr.read_u16());
    let mode_mapping = r#try!(rdr.read_u8());
    // Verifying ranges
