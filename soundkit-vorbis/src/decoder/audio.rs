...
// Modified by SoundKit in 2026 for in-tree streaming decode.
...
use super::header::{
    Codebook, Floor, FloorTypeOne, FloorTypeZero, HuffmanVqReadErr, IdentHeader, Mapping, Residue,
    SetupHeader,
};
use super::ilog;
...
impl fmt::Display for AudioReadError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        let description = match self {
            ...
            AudioReadError::BufferNotAddressable => {
...
impl From<HuffmanVqReadErr> for FloorSpecialCase {
    fn from(e: HuffmanVqReadErr) -> Self {
        ...
        use super::header::HuffmanVqReadErr::*;
        match e {
            EndOfPacket => Unused,
...
fn floor_zero_decode(
    ...
    let amplitude = r#try!(rdr.read_dyn_u64(fl.floor0_amplitude_bits));
    if amplitude <= 0 {
    ...
    let booknumber = r#try!(rdr.read_dyn_u32(ilog(fl.floor0_number_of_books as u64)));
    match fl.floor0_book_list.get(booknumber as usize) {
        // Undecodable per spec
        None => r#try!(Err(FloorSpecialCase::PacketUndecodable)),
        Some(codebook_idx) => {
            ...
            loop {
                ...
                let temp_vector = r#try!(rdr.read_huffman_vq(codebook));
...
fn floor_one_decode(
    ...
    let b = ilog(range - 1);
    floor1_y.push(r#try!(rdr.read_dyn_u8(b)) as u32);
    floor1_y.push(r#try!(rdr.read_dyn_u8(b)) as u32);
    ...
    for class in &fl.floor1_partition_class {
        ...
        if cbits > 0 {
            ...
            cval = r#try!(rdr.read_huffman(&codebooks[cbook].codebook_huffman_tree));
        }
        for _ in 0..cdim {
            ...
            if book >= 0 {
                ...
                floor1_y.push(r#try!(rdr.read_huffman(tree)));
            } else {
                floor1_y.push(0);
            }
        }
    ...
    for i in 0..ident.audio_channels as usize {
        ...
        let floor_res = match floor {
            &Floor::TypeZero(ref fl) => match floor_zero_decode(rdr, codebooks, fl) {
                ...
                Err(PacketUndecodable) => r#try!(Err(())),
            },
...
fn residue_packet_read_partition(
    ...
    if resid.residue_type == 0 {
        ...
        for i in 0..step {
            let entry_temp = r#try!(rdr.read_huffman_vq(codebook));
        ...
        while i < partition_size {
            let entries = r#try!(rdr.read_huffman_vq(codebook));
            ...
            while partition_count < partitions_to_read {
                ...
                for _ in 0..classwords_per_codeword {
                    ...
                    for (j, do_not_decode) in do_not_decode_flag.iter().enumerate() {
                        ...
                        let offs = limit_residue_begin
                            + partition_count * resid.residue_partition_size as usize;
                        let vec_j_offs =
                            &mut vectors[(j * actual_size + offs)..((j + 1) * actual_size)];
...
fn residue_packet_decode(
    ...
    if resid.residue_type == 2 {
        ...
        if !to_decode_found {
            ...
            let vectors = r#try!(residue_packet_decode_inner(
                rdr,
        ...
        return residue_packet_decode_inner(
            rdr,
            cur_blocksize,
            do_not_decode_flag,
            resid,
...
pub fn get_decoded_sample_count(
    ...
    if r#try!(rdr.read_bit_flag()) {
        r#try!(Err(AudioReadError::AudioIsHeader));
    }
    let mode_number = r#try!(rdr.read_dyn_u8(ilog(setup.modes.len() as u64 - 1)));
    ...
    let previous_next_window_flag = if mode.mode_blockflag {
        Some((r#try!(rdr.read_bit_flag()), r#try!(rdr.read_bit_flag())))
    } else {
        None
    };
...
pub fn read_audio_packet_generic<S: Samples>(
    ...
    let mode = if let Some(mode) = setup.modes.get(mode_number as usize) {
        ...
        r#try!(Err(AudioReadError::AudioBadFormat))
    };
    ...
    // Decode the floors
    let decoded_floor_infos = r#try!(floor_decode(
        &mut rdr,
        ident,
        mapping,
        &setup.codebooks,
    ...
    for (channel, floor) in decoded_floor_infos.iter().enumerate() {
        no_residue[channel] = floor.is_unused();
    }
    // and also propagate
    for (&mag, &angle) in mapping
        .mapping_magnitudes
        .iter()
        .zip(mapping.mapping_angles.iter())
    {
    ...
    for (i, &residue_number) in mapping.mapping_submap_residues.iter().enumerate() {
        let mut do_not_decode_flag = [false; 256];
        let mut submap_channels = 0usize;
        for (j, &mapping_mux_j) in mapping.mapping_mux.iter().enumerate() {
            if mapping_mux_j as usize == i {
                do_not_decode_flag[submap_channels] = no_residue[j];
                submap_channels += 1;
            }
        }
        let cur_residue = &setup.residues[residue_number as usize];
        let vectors = match residue_packet_decode(
            &mut rdr,
            n,
            &do_not_decode_flag[..submap_channels],
            cur_residue,
            &setup.codebooks,
        ) {
            Ok(v) => v,
    ...
    {
        let (mag_vector, angle_vector) =
            dual_mut_idx(&mut residue_vectors, mag as usize, angle as usize);
    ...
    for ref mut spectrum in audio_spectri.iter_mut() {
        ...
        super::fast_imdct::inverse_mdct(cached_bd, &mut spectrum[..]);
    ...
    if let Some(prev_data) = pwr.data.take() {
        ...
        for chan in audio_spectri.iter_mut() {
            let mut future_prev_half =
                Vec::with_capacity((right_win_end - right_win_start) as usize);
