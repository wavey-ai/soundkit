...
// Modified by SoundKit in 2026 for the in-tree decoder core.
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        fn fmt_rec(s :&HuffTree, f: &mut fmt::Formatter, depth :u32) -> fmt::Result {
            ...
            if s.l.is_some() || s.r.is_some() {
                r#try!(writeln!(f, "ec: {:?}, pl: {:?}, LIS {:?} RIS {:?}",
                    s.even_childs, s.payload, s.l.is_some(), s.r.is_some()));
            } else {
                r#try!(writeln!(f, "ec: {:?}, pl: {:?}", s.even_childs, s.payload));
            }
            if let Some(ref v) = s.l {
                depth_print!(f, depth);
                r#try!(write!(f, "LEFT "));
                r#try!(fmt_rec(&*v, f, depth + 1));
            }
...
impl VorbisHuffmanTree {
    ...
    pub fn load_from_array(
        codebook_codeword_lengths: &[u8],
    ) -> Result<VorbisHuffmanTree, HuffmanError> {
        ...
        for (i, &codeword_length) in codebook_codeword_lengths.iter().enumerate() {
            ...
            if !simple_tree.insert_rec(i as u32, codeword_length) {
                r#try!(Err(HuffmanError::Overspecified)) /* Overspecified, can't be put into tree */
            }
        }
        //println!("The tree:\n{:?}", simple_tree);
        ...
        if cnt == 1 {
            ...
            if encoded_len == 1 {
                ...
                r#try!(Err(HuffmanError::InvalidSingleEntry))
            }
        }

        if !simple_tree.even_childs {
            r#try!(Err(HuffmanError::Underpopulated)); /* Underpopulated */
        }
