...
function makePage({ caseName, bitrate, seconds, frameSize, pcmBits, cacheBust, profileMode }) {
  return `<!doctype html>
<meta charset="utf-8">
<title>soundkit-opus browser wasm benchmark</title>
...
async function loadRust() {
  const start = performance.now();
  const module = await import("/rust/soundkit_opus.js?v=" + CACHE_BUST);
  const imported = performance.now();
  const wasm = await module.default({ module_or_path: "/rust/soundkit_opus_bg.wasm?v=" + CACHE_BUST });
...
async function benchRust(module, wasm, fixture, vbr, reuseOutput, loadMs) {
  ...
  for (let frame = 0; frame < fixture.frames; frame += 1) {
    ...
    if (reuseOutput) {
      ...
      if (encodedSize <= 0 || encodedSize !== encoder.outputLen) {
        throw new Error("soundkit-opus reuse encode failed at frame " + frame + ": " + encodedSize);
      }
      ...
      if (!result.ok) {
        throw new Error("soundkit-opus encode failed at frame " + frame);
      }
      packets.push(result.encodedData);
      result.free();
    }
  ...
  for (let frame = 0; frame < packets.length; frame += 1) {
    if (reuseOutput) {
      ...
      if (decodedSize !== FRAME_SIZE) {
        throw new Error("soundkit-opus reuse decode failed at frame " + frame + ": " + decodedSize);
      }
      ...
      if (result.decodedSize !== FRAME_SIZE) {
        throw new Error("soundkit-opus decode failed at frame " + frame + ": " + result.decodedSize);
      }
  ...
  decoder.destroy();
  return finish(
    "Rust soundkit-opus " + (vbr ? "VBR" : "CBR") +
...
function createAssetServer(paths) {
  const server = createServer((request, response) => {
    ...
    const asset = {
      "/rust/soundkit_opus.js": ["text/javascript; charset=utf-8", paths.rustJs],
      "/rust/soundkit_opus_bg.wasm": ["application/wasm", paths.rustWasm],
...
async function main() {
  ...
  const paths = {
    rustJs: resolve(options.rustPkg, "soundkit_opus.js"),
    rustWasm: resolve(options.rustPkg, "soundkit_opus_bg.wasm"),
    cJs: resolve(options.libopusjs, "libopus.mjs"),
    cWasm: resolve(options.libopusjs, "libopus.wasm"),
  };

  requireFile(options.chrome, "Chrome");
  requireFile(paths.rustJs, "soundkit-opus JS glue");
  requireFile(paths.rustWasm, "soundkit-opus wasm");
