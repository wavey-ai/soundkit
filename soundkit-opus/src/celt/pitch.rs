...
fn pitch_xcorr(x: &[f32], y: &[f32], len: usize, max_pitch: usize, xcorr: &mut Vec<f32>) {
    ...
    #[cfg(target_arch = "x86_64")]
    let used_native_simd = crate::kernels::pitch_xcorr_8(x, y, len, grouped, &mut xcorr[..grouped]);
    #[cfg(target_arch = "aarch64")]
    let used_native_simd = crate::kernels::pitch_xcorr_4(x, y, len, grouped, &mut xcorr[..grouped]);
...
fn dual_inner_prod(x: &[f32], y01: &[f32], y02: &[f32], n: usize) -> (f32, f32) {
    ...
    if let Some(result) = crate::kernels::dual_inner_prod_f32(x, y01, y02, n) {
        return result;
    }
