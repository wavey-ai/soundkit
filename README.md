...
| 7 | **GSM 06.10** | Legacy PBX and call-recording codec. Useful for archive compatibility rather than new integrations. | Implemented as `soundkit-gsm` using the pure-Rust `oxideav-gsm` codec core, with standard raw `.gsm` and Microsoft WAV-49 framing. The dependency carries MIT plus libgsm notice terms, so packaged distributions should preserve those notices. |
