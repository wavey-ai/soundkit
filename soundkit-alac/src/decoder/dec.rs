...
use super::{invalid_data, InvalidData, StreamInfo};
...
fn decode_audio_element<'a, S: Sample>(
    ...
    let num_samples = if partial_frame {
        ...
        if num_samples == 0 || num_samples > this.config.frame_length {
            return Err(invalid_data("channel contains an invalid sample count"));
        }
    ...
    if !is_uncompressed {
        ...
        let chan_bits = this
            .config
            .bit_depth
            .checked_sub(sample_shift)
            .and_then(|bits| bits.checked_add(element_channels - 1))
            .filter(|bits| (1..=32).contains(bits))
            .ok_or_else(|| invalid_data("channel bit depth is outside the supported range"))?;
        ...
        for i in 0..(element_channels as usize) {
            ...
            if lpc_quant[i] == 0 || usize::from(lpc_order[i]) >= num_samples {
                return Err(invalid_data("invalid lpc quantization or order"));
            }
        ...
        for i in 0..(element_channels as usize) {
            rice_decompress(reader, &this.config, mix_buf[i], chan_bits, pb_factor[i])?;

            if lpc_mode[i] == 15 {
                // the special "numActive == 31" mode can be done in-place
                lpc_predict_order_31(mix_buf[i], chan_bits);
            } else if lpc_mode[i] > 0 {
...
fn decode_rice_symbol<'a>(
    ...
    // after Q.
    let q = reader.read_unary_ones_max9()?;
    ...
    while i < buf.len() {
        ...
        if (rice_history < 128) && (i + 1 < buf.len()) {
            ...
            if zero_block_len > 0 {
                ...
                buf[i + 1..i + 1 + zero_block_len].fill(0);
                i += zero_block_len;
            }
...
fn lpc_predict(buf: &mut [i32], bps: u8, lpc_coefs: &mut [i16], lpc_quant: u32) {
    ...
    for i in (lpc_order + 1)..buf.len() {
        ...
        if prediction_error > 0 {
            // Splitting positive and negative errors once per sample removes
            // the error-sign multiplications from this coefficient hot loop.
            let mut remaining_error = prediction_error;
            for j in 0..lpc_order {
                let delta = buf[j].wrapping_sub(mean);
                let sign = sign_only(delta);
                lpc_coefs[j] = lpc_coefs[j].wrapping_add(sign as i16);
                let magnitude = delta.wrapping_mul(sign);
                remaining_error = remaining_error
                    .wrapping_sub((magnitude >> lpc_quant).wrapping_mul(j as i32 + 1));
                if remaining_error <= 0 {
                    break;
                }
            }
        } else if prediction_error < 0 {
            let mut remaining_error = prediction_error.wrapping_neg();
            for j in 0..lpc_order {
                let delta = buf[j].wrapping_sub(mean);
                let sign = sign_only(delta);
                lpc_coefs[j] = lpc_coefs[j].wrapping_sub(sign as i16);
                let magnitude = delta.wrapping_mul(sign);
                // Arithmetic right shift rounds negative values down, so a
                // negative prediction error uses ceil(magnitude / 2^quant).
                let correction = (magnitude.wrapping_neg() >> lpc_quant).wrapping_neg();
                remaining_error =
                    remaining_error.wrapping_sub(correction.wrapping_mul(j as i32 + 1));
                if remaining_error <= 0 {
                    break;
                }
            }
...
fn append_extra_bits<'a>(
    ...
    for i in 0..num_samples {
        for j in 0..channels {
            ...
            buf[j][i] = (buf[j][i] << sample_shift) | extra_bits;
        }
    }
